...
public class ClientOrderedList{
	...
	public static void main(String[] args){
		...
		OrderedArrayList<Integer> list_1 = new OrderedArrayList<Integer>(SIZE);
		OrderedArrayList<Integer> list_2 = new OrderedArrayList<Integer>(SIZE);
		OrderedArrayList<Integer> mergedList = new OrderedArrayList<Integer>(SIZE);
		...
		if(fileOpened) {
			while(inputFile.hasNext()) {
				if(inputFile.hasNextInt()) {
					list_1.insert(inputFile.nextInt());
				}
				else
					inputFile.next();
			}
		...
		if(fileOpened) {
			while(inputFile2.hasNext()) {
				if(inputFile2.hasNextInt()) {
					list_2.insert(inputFile2.nextInt());
				}
				else
					inputFile2.next();
			}
		...
		list_1.clearList();
		list_2.clearList();
		list_2 = mergedList.split(list_1, listKey);
