...
public abstract class BinaryTree<T> implements BinaryTreeADT<T> { 
	...
	public boolean isBST(BinaryTreeNode<T> tree) {
		...
		if(tree.rLink == null || tree.lLink == null)
			...
			Comparable<T> temp = (Comparable<T>) tree.lLink.info;
			if(temp.compareTo(tree.rLink.info) > 0)
				return false;
			else if(!isBST(tree.lLink))
				return false;
			else
				return isBST(tree.rLink);
		}
	}
	...
	public boolean similar(BinaryTreeNode<T> tree1, BinaryTreeNode<T> tree2) {
		if(tree1 == null || tree2 == null) {
			...
			Comparable<T> temp = (Comparable<T>) tree1.info;
			if(temp.compareTo(tree2.info) != 0)
				return false;
			else if(similar(tree1.lLink, tree2.lLink))
				return true;
			else
				return similar(tree1.rLink, tree2.rLink);
